...
import { ArrowLeft, ArrowUpRight, BookMarked, Check, CircleAlert, Copy, Download, FileCheck2, FileText, Landmark, Printer, Scale, ShieldCheck } from "lucide-react";
...
async function copyReference(content: string) {
  ...
  textarea.remove();
  if (!copied) throw new Error("Não foi possível copiar a referência.");
}
...
export default function CitationDossierPage() {
  ...
  const reference = buildPublicCitationReference({ theme: record.theme, tribunal: record.tribunal, cnjNumber: record.cnjNumber, externalId: record.externalId, decisionDate: record.decisionDate, sourceUrl: source.sourceUrl });
  ...
  return <div className="dossier-shell">
    ...
    <main className="dossier-main">
      <header className="dossier-topbar"><a href="/compendio" className="back-to-atlas"><ArrowLeft size={16} /> Compêndio jurídico</a><div><button className={`copy-reference-button${copied ? " copied" : ""}`} onClick={handleCopyReference}>{copied ? <Check size={15} /> : <Copy size={15} />}{copied ? " Referência copiada" : " Copiar referência"}</button><button onClick={() => downloadDossier(`dossie-${record.externalId}.md`, markdown)}><Download size={15} /> Baixar Markdown</button><button onClick={() => window.print()}><Printer size={15} /> Imprimir</button></div></header>
