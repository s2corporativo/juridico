...
export default function PublicSourcesPage() {
  ...
  const catalog = trpc.sources.stjCatalog.useQuery({ query });
  ...
  return <div className="sources-shell">
    ...
    <main className="sources-main">
      <header className="sources-topbar"><a href="/estrutura" className="back-to-atlas"><ArrowLeft size={16} /> Estrutura interna</a><a href="/compendio" className="sources-compendium-link">Compêndio jurídico <ArrowUpRight size={15} /></a></header>
      ...
      <section className="sources-matrix" id="matriz"><div className="sources-section-heading"><Landmark size={20} /><div><span>CATÁLOGO INSTITUCIONAL</span><h2>Fontes públicas priorizadas.</h2></div></div><div className="source-card-grid">{sources.data.map(source => <article className="source-card" key={source.id}><div><span className={`source-status ${source.integrationStatus}`}>{statusLabel[source.integrationStatus]}</span><small>{source.sourceType}</small></div><h3>{source.label}</h3><p className="source-maintainer">{source.maintainer}</p><p>{source.contentScope}</p><dl><div><dt>Cobertura</dt><dd>{source.coverage}</dd></div><div><dt>Acesso</dt><dd>{source.authentication === "api_key" ? "API Key" : source.authentication === "manual" ? "Consulta humana" : "Sem credencial"}</dd></div></dl><div className="source-links"><a href={source.documentationUrl} target="_blank" rel="noreferrer">Documentação <ArrowUpRight size={13} /></a><a href={source.baseUrl} target="_blank" rel="noreferrer">Fonte <ArrowUpRight size={13} /></a></div><p className="source-note"><b>Uso:</b> {source.usageNote}</p></article>)}</div></section>
      <section className="stj-section" id="stj"><div className="stj-intro"><span className="eyebrow">CKAN · STJ DADOS ABERTOS</span><h2>Catálogo oficial consultado em tempo real.</h2><p>Esta conexão traz somente metadados do catálogo. Recursos, arquivos e decisões não são importados automaticamente.</p><label><FileSearch size={16} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Termo no catálogo do STJ" aria-label="Pesquisar no catálogo do STJ" /></label></div><div className="stj-results">{catalog.isLoading ? <p className="catalog-state">Consultando o catálogo oficial…</p> : catalog.isError ? <p className="catalog-state error"><CircleAlert size={16} /> Catálogo indisponível neste momento. Tente novamente; nenhuma fonte foi alterada.</p> : <><div className="catalog-count"><BookOpenCheck size={17} /><span>{catalog.data?.total ?? 0} conjuntos encontrados no catálogo público</span></div>{catalog.data?.entries.map(entry => <article key={entry.id}><div><span>{entry.license}</span><small>{entry.resourceCount} recursos · {entry.formats.join(" · ") || "formato não informado"}</small></div><h3>{entry.title}</h3><p>{entry.summary}</p><footer><time>Atualizado: {formatDate(entry.updatedAt)}</time><a href={entry.catalogUrl} target="_blank" rel="noreferrer">Abrir no STJ <ArrowUpRight size={13} /></a></footer></article>)}</>}</div></section>
