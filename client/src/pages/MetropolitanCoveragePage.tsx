...
export default function MetropolitanCoveragePage() {
  ...
  return <main className="min-h-screen bg-[#f4f0e8] px-4 py-6 text-[#2a2823] sm:px-8 lg:px-12">
    <header className="mx-auto flex max-w-7xl flex-col gap-5 border-b border-[#d5ccbd] pb-6 sm:flex-row sm:items-start sm:justify-between">
      <div><a href="/" className="mb-5 inline-flex items-center gap-2 text-sm font-semibold text-[#1a5652]"><ArrowLeft size={16} /> Voltar ao Atlas</a><p className="text-xs font-bold tracking-[0.19em] text-[#a16632]">COBERTURA TERRITORIAL · RMBH</p><h1 className="mt-2 font-serif text-4xl leading-none sm:text-5xl">Órgãos, território e <em className="text-[#13514e]">proveniência.</em></h1><p className="mt-4 max-w-3xl text-sm leading-6 text-[#60584e]">Mapa de órgãos JEC do TJMG associados por rótulo exato a municípios da Região Metropolitana de Belo Horizonte. Esta camada não consulta nem exibe processos individuais.</p></div>
    ...
    {data && <div className="mx-auto max-w-7xl py-8">
      ...
      <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {municipalities.map(item => <article key={item.ibgeCode} className={`border p-5 ${item.state === "mapped" ? "border-[#b9cfc9] bg-[#f7fbf9]" : "border-[#d7cfc2] bg-[#fbf8f2]"}`}>
          ...
          {item.state === "mapped" ? <><div className="mt-5 grid grid-cols-2 gap-3 border-y border-[#d4e1dc] py-3"><div><span className="block text-[11px] font-bold tracking-wider text-[#60736e]">ÓRGÃOS</span><strong className="text-xl text-[#13514e]">{fmt.format(item.bodyCount)}</strong></div><div><span className="block text-[11px] font-bold tracking-wider text-[#60736e]">SOMA DE FACETAS</span><strong className="text-xl text-[#13514e]">{fmt.format(item.facetAmount)}</strong></div></div><ul className="mt-4 space-y-2 text-xs leading-5 text-[#514a42]">{item.bodies.slice(0, 3).map(body => <li key={body.code}><b className="text-[#13514e]">{body.code}</b> · {body.label} <span className="text-[#7d6e5e]">({fmt.format(body.amount)})</span></li>)}{item.bodies.length > 3 && <li className="text-[#7d6e5e]">+ {item.bodies.length - 3} órgão(s) no recorte.</li>}</ul></> : <p className="mt-5 border-t border-[#e1d7ca] pt-4 text-sm leading-6 text-[#685d50]">Sem órgão com vínculo literal à comarca nesta execução. Não se presume cobertura por município-sede, proximidade territorial ou faceta nacional sem alias.</p>}
        </article>)}
      </section>

      <section className="mt-8 grid gap-4 border border-[#13514e] bg-[#123f3e] p-5 text-[#f4f0e8] lg:grid-cols-[auto_1fr]"><ShieldCheck size={28} className="text-[#e3b778]" /><div><h2 className="font-serif text-2xl">Limite de leitura</h2><p className="mt-2 max-w-4xl text-sm leading-6 text-[#d8e2dc]">{data.readiness.coverageNote} A soma de facetas de órgão não deve ser lida como processos únicos, estoque, produtividade, taxa de êxito ou comparação completa entre municípios. Municípios sem órgão confirmado permanecem visíveis como lacuna metodológica.</p><p className="mt-3 text-xs font-bold tracking-[0.1em] text-[#e3b778]"><Landmark size={13} className="mr-1 inline" /> Fonte: CNJ/DataJud API Pública · TJMG · consulta agregada `size=0`.</p></div></section>
    </div>}
  </main>;
}
